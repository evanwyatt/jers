...
#define CMP_INT(__n) if (a->__n != b->__n) { printf("Field '%s' does not match.\n", #__n); return 1;}
#define CMP_STR(__n) if (cmp_str(a->__n, b->__n)) { printf("Field '%s' does not match. a = '%s' b = '%s'\n", #__n, a->__n, b->__n); return 1;}
#define CMP_STRARRAY(__n,__c) do { \
